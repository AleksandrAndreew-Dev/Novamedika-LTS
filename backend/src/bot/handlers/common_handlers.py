...
# В функции handle_user_message изменяем логику:
...
async def handle_user_message(
    ...
    if current_state is not None:
        logger.debug(f"User {user.telegram_id} in state {current_state}, skipping direct processing")
        return

    # Если пользователь фармацевт - показываем панель
    if is_pharmacist and pharmacist:
        status_text = "🟢 онлайн" if pharmacist.is_online else "🔴 офлайн"
        await message.answer(
            f"👨‍⚕️ <b>Панель фармацевта</b>\n\n"
        ...
        # Для обычных пользователей - отправляем текст как вопрос
        # (но это уже сделано в user_questions.py, поэтому просто показываем инструкцию)
        await message.answer(
            "👋 <b>Просто напишите ваш вопрос!</b>\n\n"
            "Примеры вопросов:\n"
            "• 'Какое лекарство от головной боли лучше?'\n"
            "• 'Можно ли принимать парацетамол с антибиотиками?'\n"
            "• 'Какие аналоги у препарата Ибуфен?'\n\n"
