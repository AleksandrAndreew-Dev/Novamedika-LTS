"""API для внешних аптек: регистрация, логин, конфигурация, orders callback."""
...
async def register_pharmacy(
    ...
    """Регистрация новой аптеки — генерация токена для pull-модели"""
    import secrets
...
async def external_order_callback(request: Request, db: AsyncSession = Depends(get_db)):
    ...
    Body JSON: external_order_id, local_order_id, status, reason
    """
    from routers.orders_helpers import (
        authenticate_pharmacy_api_config,
    ...
    api_config = await authenticate_pharmacy_api_config(db, token)
