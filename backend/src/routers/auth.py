...
async def register_user(
    ...
    try:
        ...
        new_user = User(
            ...
            consent_privacy_policy_date=get_utc_now_naive() if user_data.consent_privacy_policy else None,
            consent_transboundary_transfer=user_data.consent_transboundary_transfer,
            consent_transboundary_transfer_date=get_utc_now_naive() if user_data.consent_transboundary_transfer else None,
        ...
        raise HTTPException(
            status_code=500,
            detail="Ошибка регистрации. Попробуйте позже."
        )
...
async def login_user(
    ...
    try:
        ...
        user = await get_user_by_email_or_phone(
            login_data.email, login_data.phone, db
        )
        ...
        raise HTTPException(
            status_code=500,
            detail="Ошибка входа. Попробуйте позже."
        )
