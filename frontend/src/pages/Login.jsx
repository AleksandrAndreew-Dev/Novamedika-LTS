...
export default function Login() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const errorMessage =
        err.userMessage || err.message || "Ошибка входа. Попробуйте позже.";
  ...
  const handleTelegramLogin = async () => {
    ...
    try {
      ...
      if (!telegramAuthService.isInTelegram) {
        setToast({
          message: "Эта функция доступна только в приложении Telegram",
          type: "info",
        });
      ...
      console.log("[Login] Attempting Telegram WebApp login");
      const success = await telegramAuthService.autoLogin();

      if (success) {
        console.log("[Login] ✅ Telegram login successful");
        setToast({ message: "Вы вошли через Telegram!", type: "success" });
        ...
        console.log("[Login] Telegram login failed");
        setError("Ошибка входа через Telegram. Попробуйте позже.");
        setToast({ message: "Ошибка входа через Telegram", type: "error" });
      }
    } catch (err) {
      console.error("[Login] Telegram login error:", err);
      const errorMessage = err.message || "Ошибка входа через Telegram";
      setError(errorMessage);
      setToast({ message: errorMessage, type: "error" });
    } finally {
      setTelegramLoading(false);
