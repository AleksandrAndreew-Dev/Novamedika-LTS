...
export default function AskPharmacist({ onClose }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (!anonUserId) {
        anonUserId = crypto.randomUUID ? crypto.randomUUID() : "anon-" + Date.now() + "-" + Math.random().toString(36).slice(2);
      ...
      history.push({ uuid: data.uuid, text: data.text, created_at: new Date().toISOString() });
    ...
    <>
      <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center p-4 z-50" onClick={onClose}>
        <div className="bg-white rounded-3xl shadow-2xl max-w-md w-full p-6 relative" onClick={(e) => e.stopPropagation()}>
          {/* Close button */}
          <button onClick={onClose} className="absolute top-4 right-4 p-1 text-gray-400 hover:text-gray-600 hover:bg-gray-100 rounded-full transition-colors" aria-label="Закрыть">
          ...
          {result ? (
            ...
            <div className="text-center">
              ...
              <h3 className="text-lg font-semibold text-gray-900 mb-1">Вопрос отправлен!</h3>
              <p className="text-sm text-gray-500 mb-4">Фармацевт ответит в ближайшее время. Вы можете задать ещё вопрос.</p>
              <button onClick={() => { setResult(null);
                  setQuestionText(""); }} className="bg-blue-600 hover:bg-blue-700 text-white font-medium py-2.5 px-6 rounded-full transition-colors text-sm">
                Задать ещё вопрос
              </button>
            </div>
          ) : (
            /* Form state */
            <>
              <h3 className="text-lg font-semibold text-gray-900 text-center mb-1">Задать вопрос фармацевту</h3>
              <p className="text-sm text-gray-500 text-center mb-5">Опишите ваш вопрос, и фармацевт ответит вам</p>
              ...
              <p className="text-xs text-gray-400 text-center mt-4">
                Нажимая "Отправить", вы соглашаетесь с обработкой данных согласно <a href="/privacy-policy" target="_blank" className="text-blue-500 underline">Политике конфиденциальности</a>
              </p>
            </>
          )}
        </div>
