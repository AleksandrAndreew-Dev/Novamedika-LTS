...
export default function ConsultationChat({
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      try {
        ...
        setError("Не удалось загрузить данные консультации");
      } finally {
        setLoading(false);
      }
    };
  ...
  const getUserName = () => {
    ...
    return user?.first_name || user?.telegram_username || `Пользователь #${user?.telegram_id?.toString().slice(-4) || ""}`;
  };
  ...
  const getUserColor = () => {
    const colors = ["#16a34a", "#ea580c", "#7c3aed", "#ec4899", "#2563eb", "#0891b2", "#65a30d"];
    const user = question?.user;
    const seed = user?.telegram_id || user?.uuid || 0;
    // Convert to number for color selection
    const num = typeof seed === "string" ? seed.split("").reduce((a, c) => a + c.charCodeAt(0), 0) : Number(seed);
  ...
  const getStatusBadge = () => {
    ...
    const badges = {
      pending: { text: "В ожидании", class: "bg-yellow-100 text-yellow-700" },
      answered: { text: "Отвечено", class: "bg-green-100 text-green-700" },
      completed: { text: "Завершено", class: "bg-gray-100 text-gray-500" },
      in_progress: { text: "В работе", class: "bg-blue-100 text-blue-700" },
    };
    const badge = badges[question.status] || badges.pending;
    return <span className={`px-2.5 py-0.5 rounded-full text-[11px] font-medium ${badge.class}`}>{badge.text}</span>;
  };
  ...
  if (!questionId) {
    return (
      <div className="flex h-full flex-col items-center justify-center bg-gray-50 p-6 text-center">
        <svg width="160" height="112" viewBox="0 0 200 140" fill="none" className="opacity-30 mb-4">
          <rect x="10" y="20" width="180" height="100" rx="12" stroke="#d1d5db" strokeWidth="2" fill="#f9fafb"/>
          <rect x="25" y="35" width="60" height="8" rx="4" fill="#e5e7eb"/>
          <rect x="25" y="50" width="100" height="6" rx="3" fill="#e5e7eb"/>
          <rect x="25" y="62" width="80" height="6" rx="3" fill="#e5e7eb"/>
          <rect x="140" y="80" width="40" height="8" rx="4" fill="#3b82f6" opacity="0.3"/>
        </svg>
        <p className="text-base font-semibold text-gray-800">Выберите консультацию</p>
        <p className="mt-1.5 text-sm text-gray-500">Чат откроется в этой панели</p>
      </div>
    );
  }
    ...
    return (
      <div className="flex h-full items-center justify-center bg-gray-50 p-6">
        <div className="flex items-center gap-3 text-gray-400">
          ...
          <span className="text-sm">Загрузка диалога...</span>
        </div>
      </div>
    );
  }
  ...
  if (error && !question) {
    return (
      <div className="flex h-full flex-col items-center justify-center bg-red-50 p-6 text-center">
        ...
        <button onClick={handleBack} className="mt-3 px-4 py-1.5 text-xs font-medium bg-white border border-red-200 rounded-full text-red-600 hover:bg-red-50">
          Вернуться
        </button>
      </div>
    );
  ...
  return (
    <div className="flex h-full flex-col overflow-hidden bg-gray-100 rounded-2xl border border-gray-200 shadow-sm">
      ...
      <div className="bg-white px-4 py-3 flex items-center gap-3 border-b border-gray-200 flex-shrink-0">
        <button onClick={handleBack} className="p-1 text-gray-400 hover:text-gray-600 hover:bg-gray-100 rounded-full transition-colors flex-shrink-0" aria-label="Назад">
        ...
        <div
          ...
          style={{ background: `linear-gradient(135deg, ${getUserColor()}, ${getUserColor()}dd)` }}
        >
          {getUserAvatar()}
        ...
        <div className="flex-1 min-w-0">
          <div className="font-semibold text-gray-900 text-sm tracking-wide truncate">{getUserName()}</div>
          <div className="flex items-center gap-2 mt-0.5">
            <span className="text-[11px] text-gray-400">
              {question?.status === "pending" ? "ожидает ответа" : question?.status === "answered" ? "есть ответ" : question?.status}
            </span>
            {getStatusBadge()}
          </div>
        </div>
      ...
      {/* ===== MESSAGES ===== */}
      <div className="flex-1 overflow-y-auto px-4 py-4 flex flex-col gap-0.5 scroll-smooth" onScroll={handleScroll}>
        {messages.length === 0 ? (
          <div className="flex-1 flex items-center justify-center">
            <div className="text-center">
              ...
              <p className="text-gray-500 text-sm">Диалог пока пуст</p>
          ...
          messages.map((msg, idx) => {
            ...
            return (
              <div key={msg.uuid || idx} className={`flex ${isPharmacist ? "justify-end" : "justify-start"} mb-0.5 animate-fadeIn`}>
                ...
                <div className={`max-w-[82%] ${isPharmacist ? "order-1" : "order-2"}`}>
                  {!isPharmacist && showAvatar && (
                    <div className="text-[11px] font-semibold text-blue-600 mb-0.5 tracking-wide">{getUserName()}</div>
                  )}
                  <div className={`px-3.5 py-2.5 rounded-2xl ${
                    ...
                    <div className="text-[14px] leading-relaxed whitespace-pre-wrap break-words">{msg.text}</div>
                    <div className={`flex items-center justify-end gap-1 mt-1 ${isPharmacist ? "text-gray-400" : "text-gray-400"}`}>
      ...
      {!isAtBottom && messages.length > 0 && (
        <button onClick={scrollToBottom} className="absolute bottom-24 right-6 w-8 h-8 bg-white border border-gray-200 rounded-full shadow-md flex items-center justify-center z-10 hover:shadow-lg" aria-label="Вниз">
      ...
      <div className="bg-white px-3 py-2.5 border-t border-gray-200 flex-shrink-0">
        ...
        <div className="flex items-end gap-2">
          <div className="flex-1 bg-gray-50 border border-gray-200 rounded-2xl px-4 py-1 focus-within:border-blue-500 focus-within:ring-2 focus-within:ring-blue-100 transition-all">
            <textarea
              ...
              disabled={sending || question?.status === "completed"}
            />
          </div>
          <button
            onClick={handleSend}
            disabled={sending || !newMsg.trim() || question?.status === "completed"}
        ...
        {question?.status === "completed" && (
          <p className="text-xs text-gray-400 text-center mt-2">Консультация завершена</p>
        )}
      </div>
